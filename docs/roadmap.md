...
最后更新：2026-06-17（一轮代码审查驱动的优化共 20 项落地，见下「已落地优化 2026-06-16 / 06-17（一/二/三）」四节：记忆并发+异步 / chunking / prompt caching+指标 / eval 三类黄金集+baseline CI / nl2sql 数字 grounding+自修上限 / tools 错误处理 / 流式 PII 后处理 / A2A input-required+断连取消）。同步落到 `CLAUDE.md`、`docs/nl2sql.md`(2.B)、`docs/a2a.md`(流式收口)、`application.yml`。
